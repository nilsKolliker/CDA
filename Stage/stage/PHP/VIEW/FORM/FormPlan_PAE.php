...
if (isset($_GET['id'])) {
	$elm = Plan_PAEManager::findById($_GET['id']);
} else {
	$elm = new Plan_PAE();
}
echo '<main class="center">';

echo '<form class="GridForm" action="index.php?page=ActionPlan_PAE&mode='.$_GET['mode'].'" method="post"/>';

echo '<div class="caseForm titreForm col-span-form">Formulaire Plan_PAE</div>';
	echo '<div class="noDisplay"><input type="hidden" value="'.$elm->getIdPAE().'" name=IdPAE></div>';
...
echo '<div class="caseForm col-span-form">
	<div></div>
	<div><a href="index.php?page=ListePlan_PAE"><button type="button"><i class="fas fa-sign-out-alt fa-rotate-180"></i></button></a></div>
