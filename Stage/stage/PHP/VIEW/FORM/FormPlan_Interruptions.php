...
if (isset($_GET['id'])) {
	$elm = Plan_InterruptionsManager::findById($_GET['id']);
} else {
	$elm = new Plan_Interruptions();
}
echo '<main class="center">';

echo '<form class="GridForm" action="index.php?page=ActionPlan_Interruptions&mode='.$_GET['mode'].'" method="post"/>';

echo '<div class="caseForm titreForm col-span-form">Formulaire Plan_Interruptions</div>';
	echo '<div class="noDisplay"><input type="hidden" value="'.$elm->getIdInterruption().'" name=IdInterruption></div>';
...
echo '<div class="caseForm col-span-form">
	<div></div>
	<div><a href="index.php?page=ListePlan_Interruptions"><button type="button"><i class="fas fa-sign-out-alt fa-rotate-180"></i></button></a></div>
