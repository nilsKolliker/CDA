...
{
	$retour=Plan_TextesManager::findByCodes($_SESSION['lang'], $codeTexte);
...
/**
 * crée un select à partir d'un tableau
 * @param integer $valeur => la cle de l'element a Selectionner
 * @param array $liste => le tableau à utiliser pour le select
 * @param string $nomDuSelect => le nom à donner au select
 * @param string $attributs => les attributs du select
 */
function CreerSelectByArray(?int $valeur, array $liste, string $nomDuSelect, ?string $attributs = ""){
	$select = '<select id="' . $nomDuSelect . '" name="' . $nomDuSelect . '"' . $attributs . '>';
	if ($valeur === null) {
		$select .= '<option value="" SELECTED>'.texte("inputDefault").'</option>';
	} else {
		$select .= '<option value="">'.texte("inputDefault").'</option>';
	}
	foreach ($liste as $key => $elt) {
		if ($valeur === $key) {
